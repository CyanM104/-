...
plt.plot(wavelengths_A, F_num, 'b-', linewidth=3, label='Numerical Integration (Rigorous)')
plt.plot(wavelengths_A, F_ana, 'orange', linestyle='--', linewidth=3, label=r'Analytic Approx. ($\mu = 0.6$ fixed)')
plt.plot(wavelengths_A, F_rest, 'g:', linewidth=3, label=r'Rest Frame Blackbody ($\beta = 0$)')

plt.title(rf"Relativistic Blackbody Models ($T' = {T_prime:.0f}$K, $\beta = {beta:.2f}$)", fontsize=16)
