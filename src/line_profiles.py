...
def p_cygni_line_corr_rel_1d(wl_target, vmax, vphot, tau, lam0_AA, t0, use_nlte=True, use_ltt=True):
    c_cgs = C_CGS
    ...
    f_normed = calc_rel_line_profile_with_ltt(nu_arr, lam0_AA, vmax_cgs, vphot_cgs, tau, t0, c_cgs, n_p=40, use_nlte=use_nlte, use_ltt=use_ltt)
...
def get_combined_pcygni_profile(wav, vmax, vphot, tau_sr, tau_he, t0, use_nlte, use_he, use_ltt, combine_optical_depths_beer_lambert):
    ...
    if use_he and tau_he > 0.001:
        f_he = p_cygni_line_corr_rel_1d(wav, vmax, vphot, tau_he, LAM_HE_10833_AA, t0, use_nlte=use_nlte, use_ltt=use_ltt)
    else:
